...
public interface SelectServiceDao extends JpaRepository<ViewPublishDetailEntity, Long> {
    ...
    @Query("select e from ViewPublishDetailEntity e where e.serviceType=?1 and e.price <= ?2 and e.price >=?3 and e.endDate <= ?4 and e.endDate>= ?5 and e.beginDate <= ?4 and e.beginDate >= ?5 and e.serviceName in ?6")
    public List<ViewPublishDetailEntity> findPublishEntityByCondition(String type, double upperPrice, double lowerPrice, Timestamp upperTime, Timestamp lowerTime, String[] serviceNameArr);
}
