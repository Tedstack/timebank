...
public interface SelectServiceDao extends JpaRepository<ViewPublishDetailEntity, Long> {
    ...
    @Query("select e from ViewPublishDetailEntity e where e.isDelete=0 and e.serviceType=?1 and e.price <= ?2 and e.price >=?3 and e.beginDate >= ?5 and e.endDate <= ?4 and e.serviceName in ?6 order by e.createTime")
