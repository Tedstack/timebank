...
public interface ViewRequestDetailDao extends CrudRepository<ViewRequestDetailEntity, Long> {
    ...
    @Query("select e from ViewRequestDetailEntity e where e.serviceType=?1 and e.price <= ?2 and e.price >=?3 and ((e.endTime <= ?4 and e.endTime >= ?5) or (e.beginTime <= ?4 and e.beginTime >= ?5)) and e.serviceName in ?6")
